...
export async function getSummary(projectRoot) {
    ...
    if (state.active_milestone) {
        try {
            const ms = await fm.readJson(paths.milestonePath(state.active_milestone), MilestoneSchema);
            let phasesDone = 0;
            for (const pid of ms.phases) {
                try {
                    const p = await fm.readJson(paths.phasePath(state.active_milestone, pid), PhaseSchema);
                    if (p.status === "done")
                        phasesDone++;
                }
    ...
    if (state.active_milestone && state.active_phase) {
        try {
            const ph = await fm.readJson(paths.phasePath(state.active_milestone, state.active_phase), PhaseSchema);
            let tasksDone = 0;
            for (const tid of ph.tasks) {
                try {
                    const t = await fm.readJson(paths.taskPath(state.active_milestone, state.active_phase, tid), TaskSchema);
                    if (t.status === "done")
                        tasksDone++;
                }
                catch { /* skip */ }
    ...
    if (state.active_milestone && state.active_phase && state.active_task) {
        try {
            const t = await fm.readJson(paths.taskPath(state.active_milestone, state.active_phase, state.active_task), TaskSchema);
            result.task = {
