            ...
            try {
              ...
              if (statusIdx >= STATUS_ORDER.indexOf("built") && statusIdx !== STATUS_ORDER.indexOf("build_failed")) {
                ...
                if (!(await fm.exists(reportPath))) {
                  warnings.push(
                    `Task "${taskEntry.name}": status is "${taskData.status}" but BUILDER-REPORT.md is missing`,
                  );
                }
              }
              ...
              if (statusIdx >= STATUS_ORDER.indexOf("review_passed")) {
                ...
                if (!(await fm.exists(summaryPath))) {
                  warnings.push(
                    `Task "${taskEntry.name}": status is "${taskData.status}" but reviews/review-summary.json is missing`,
                  );
                }
                const codeReviewPath = path.join(reviewsDirPath, "code-reviewer-review.md");
                if (!(await fm.exists(codeReviewPath))) {
                  warnings.push(
                    `Task "${taskEntry.name}": status is "${taskData.status}" but reviews/code-reviewer-review.md is missing`,
                  );
                }
              }
              ...
              if (statusIdx >= STATUS_ORDER.indexOf("qa_passed")) {
                ...
                if (!(await fm.exists(qaPath))) {
                  warnings.push(
                    `Task "${taskEntry.name}": status is "${taskData.status}" but reviews/qa-verification.md is missing`,
                  );
                }
              }
            } catch (err) {
