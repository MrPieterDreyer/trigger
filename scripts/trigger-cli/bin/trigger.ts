...
program
  ...
  .action(async (name: string, opts: { trust: string; description?: string }) => {
    try {
      const { config, detection } = await initProject(process.cwd(), {
        name,
        trust_level: opts.trust as "supervised" | "balanced" | "autonomous",
...
milestoneCmd
  ...
  .action(async (id: string, name: string, opts: { description?: string; branching?: string }) => {
    try {
      const milestone = await createMilestone(process.cwd(), {
        id,
        name,
        description: opts.description,
        git_branching: opts.branching as "none" | "per_phase" | "per_task" | undefined,
      });
...
phaseCmd
  ...
  .action(async (milestoneId: string, id: string, name: string, opts: { domains?: string[] }) => {
    try {
...
taskCmd
  ...
  .action(async (milestoneId: string, phaseId: string, taskId: string, newStatus: string) => {
    try {
...
taskCmd
  ...
  .action(async (milestoneId: string, phaseId: string, taskId: string) => {
    try {
