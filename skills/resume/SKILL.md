...
1. Read `.planning/state.json` — active milestone, phase, task.
2. Read the active task’s `task.json` — `status`, acceptance criteria, artifact paths.
3. Read `.planning/trigger.json` — trust level, verification commands, team defaults.
...
Resolve the active task directory from `state.json` under `.planning/milestones/…/phases/…/tasks/…` (or the layout your project uses).
