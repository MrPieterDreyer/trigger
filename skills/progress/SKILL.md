...
1. Read `.planning/state.json` (use `trigger state get`) — active milestone, phase, task, pipeline stage if recorded.
2. Read the active milestone’s `milestone.json` under `.planning/milestones/<id>/` for overall milestone progress.
