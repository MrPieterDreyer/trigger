...
3. **No active task:** `git diff` plus `git diff --staged`. **Always** spawn **code-reviewer** via `Task()` (expensive — omit `model`). Add specialist reviewers whose `activation_rules` globs match changed paths vs `.trigger/trigger.json`.
